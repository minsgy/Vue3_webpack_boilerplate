  ...
  <li>
    <div 
      ...
      <span
        ...
        @click.stop="showChildren = !showChildren">play_arrow</span>
      <span class="text">{{ workspace.title || '제목 없음' }}</span>
      <span class="actions">
        <span
          class="material-icons"
          @click.stop="createWorkspace">add</span>
        <span
          class="material-icons"
          @click.stop="deleteWorkspace">delete</span>
      </span>
    </div>
    <div
...
export default {
 ...
 created() {
   this.showChilden = this.$store.state.workspace.currentWorkspacePath
